		...
		if(_status){
			...
			db_div.innerHTML = '\
					...
					<div id="功能介绍">\
						...
						<br>　　<label>豆瓣精灵设置中心快捷键<input size=8 id="hk_db"/></label>\
						<br>　　\
						<br>　　豆瓣精灵，方便【沟通】与【获得】的工具。\
						<br>　　\
						<br>　　简单地说，使用豆瓣精灵可以更方便地“看帖”、“回帖”；\
						<br>　　更轻松地“搜索”或“下载” 相册、书籍、电影、音乐。\
						<br>　　\
						<br>　　豆瓣精灵有数十项功能，但您不需要设置。\
						<br>　　轻松地使用吧，豆瓣精灵会在您需要的地方出现。\
						<br>　　如果您是喜欢配置各种参数达人，豆瓣精灵也提供了丰富的选项。\
						<br>　　\
						<br>　　<label><input type="checkbox" id="supportHelper" />通过在卓越、淘宝商城上买书支持豆瓣精灵</label>\
					...
					<div id="相册图片" style="display:none;">\
							...
							<br>  此功能配合豆瓣精灵自带的加载全部页面功能一起使用，效果更佳。\
							<br>\
							<br>\
							贴图识别：<br>\
					...
					<div id="页面加载" style="display:none;">\
							...
							<br>  若开启“自动加载下一页”功能，浏览到网页底部时，豆瓣精灵会自动加载下一页。</p>\
							<br>\
							<br>\
							<br>\
							<br>\
					...
					<div id="搜索下载" style="display:none;">\
							...
							打开 “豆瓣精灵” 界面后看到添加其他搜索引擎文本框里有默认的如下内容：<br>\
							----------------------------------------------------------<br>\
			...
			// 			uplk.innerHTML = '<span><a href="http://www.douban.com/note/125348304/" title="豆瓣助手已更名“豆瓣精灵”，您同时安装了新版与旧版，请卸载旧版" target="_blank">豆瓣助手已更名“豆瓣精灵”，您同时安装了新版与旧版，请卸载旧版</a> <a id="forgetIt_" href=# style="float:right;" title="不再提醒">&nbsp;X&nbsp;</a></span>';
...
*/
	...
	function sw_set(ch){
		...
		if (ch){
			...
			db_write('supportHelper'); //通过在卓越、淘宝商城上买书支持豆瓣精灵
			...
			db_read('supportHelper',true);//通过在卓越、淘宝商城上买书支持豆瓣精灵
			...
			trans_code('hk_db', 'Alt+Q');//显示豆瓣精灵设置界面快捷键
			...
			if(!$('#switchHighslide')){//避免加载下一页后重复操作
				...
				photitle.innerHTML += '<br/>'
					+ (GM_getValue('highslide', false)
						...
						: '<a href="javascript:" id="switchHighslide" class="btn-donate fleft" title="点击开启高阶浏览模式，享受浏览图片的乐趣 by豆瓣精灵">开启浏览模式</a>'
						)
	...
	function down_photo(){
		$('.aside')[0].innerHTML = '\
			...
			<p><a href="javascript:void(0);" id="allPhotoPage" class="btn-donate" title="点击后打开全部大图（包含所有分页）  by豆瓣精灵"><img border="0" src="data:image/gif;base64,">读取到前<span id="loadingPage"></span>页(<span id="loadPercent"></span>)</a></p>\
			<hr>\
	...
	// function starBackup(){
		...
		// if(user){
			...
			// span.innerHTML = '<a href="javascript:void(0);" id="starBackup" class="btn-donate" title="点击后备份日记  by豆瓣精灵">[资料备份]</a>';
	...
	function sourceSearcher(){
		if(GM_getValue('sourceSearcher', true) && ((/(book|movie)\.douban\.com\/subject/.test(location.href) && $('#mainpic')) || (/read\.douban\.com\/ebook\/\d+/.test(location.href) && $('.app-article')[0]) )){
			...
			btBox.innerHTML = '<h2>资源下载&nbsp; ·&nbsp;·&nbsp;·&nbsp;·&nbsp;·&nbsp;·&nbsp;</h2>\
				...
				<span class="functionFrom">by 优酷广告杀手增强功能  </span>\
	...
	function checkIMG(link){
		...
		if (/^http.*\.(?:jpg|jpeg|jpe|jfif|bmp|png|gif|tif|tiff|ico)/i.test(href) && !/<img\s|查看原图|推荐/i.test(inner)){
			...
			link.innerHTML = '<img style="max-width:520px;" alt="图片载入ing..." title="点击鼠标中键可切换链接/图片 by豆瓣精灵" src="' + href + '" /><span style="display:none;">' + inner + '</span>';
		}
	};
	...
	function exGroup(){
		...
		if ( (ct || jv || re) && postArea && location.hostname != 'movie.douban.com'){
			...
			var re_s = re?'<a name="db_re" href="javascript:" title="回复该用户发言 by豆瓣精灵" >回</a>\
						<a name="db_qt" href="javascript:" title="引用该用户发言" >引</a> ':'';
			var re_l = re?'&gt;<a name="db_re" href="javascript:" title="回复该用户发言 by豆瓣精灵" >回复</a>&nbsp;&nbsp;\
	...
	function myGFW(){
		if(GM_getValue('myGFW', true) && $('#db-isay')){
			...
			if(!$('#myGFWbar')){
				...
				toggle.id = 'myGFWbar';
				toggle.innerHTML = '<span title="已屏蔽广播数 by 豆瓣精灵">屏蔽(<em id="blockNum">0</em>)</span> | <a id="showBlock" class="bn-status-more" href="#" title="还原显示被屏蔽广播 by 豆瓣精灵" >显示</a> | <a id="setMyGFW" class="bn-status-more" href="#" title="设置屏蔽规则 by 豆瓣精灵">设置</a> |&nbsp;';
	...
	function sortGroup(){
		if(/group\/mine/i.test(location.href) && GM_getValue('sortGroup', true)){
			...
			$$('h1')[0].innerHTML += '\<span class="gact fright">\
				...
				<span id="bydbhp" class="gact fright">分类完毕后请别忘了点击保存(by豆瓣精灵)&nbsp;&nbsp;&nbsp;</span>';
	...
	function FMdownLoad(){
		if(GM_getValue('FMdownLoad', true) && $('#fm-player')){
			...
			downDiv.innerHTML = '<span style="float:left;max-width:200px;"><a href="javascript:;" id="palyPreSong" title="播放上一首　　by豆瓣精灵">« 上一首</a></span>\
				<span style="float:right;max-width:300px;" title="Hack by 豆瓣精灵">下载：<a href="javascript:;" id="songDownLink" title="请使用下载工具，或右键另存为下载　　by豆瓣精灵" target="_blank">暂无下载</a></span>';
	...
	function slideBar(){
		if($('.global-nav-items')[0] && GM_getValue('slideBar', true)){
			...
			if(name){
				...
				myDouban.innerHTML = '<a href="http://www.douban.com/mine/" title="by 豆瓣精灵">自己</a>';
				$$('.global-nav-items ul')[0].insertBefore(myDouban, $$('.global-nav-items ul li')[0]);
			};
			
			//添加豆瓣说到导航条 因为遮挡搜索按钮暂时去除
			// if($('.top-nav-more-items')[0]){
				// $$('.top-nav-more-items ul')[0].innerHTML += '<li><a href="http://shuo.douban.com/" target="_blank" title="by 豆瓣精灵">豆瓣说</a></li>';
	...
	function musicLink(){
		...
		if(list[0] && GM_getValue('musicLink', true)){
			var getMusicLink = function(playlist){
				...
				multiDownload && (multiDownload.innerHTML = '<a href="javascript:;" title="批量下载  by豆瓣精灵" name="" onclick="var txt=document.createElement(\'textarea\');txt.style.cssText=\'width:572px;height:100px;font-size:9px;\';this.parentNode.parentNode.parentNode.appendChild(txt);txt.value=this.name;">批量下载</a>');
				for(var l,i=0,j=downLinks.snapshotLength; i<j; i++){
					l = downLinks.snapshotItem(i);
					l.innerHTML = '<a href="'+ atob(song_records[i]['url'])+'" title="右键另存为下载：'+decodeURIComponent(song_records[i]['name'])+'   by豆瓣精灵"><img src="http://t.douban.com/pics/download.gif"/></a>';
		...
		if(/douban\.com\/(?:[^\/]+\/[^\/]+|widget)\/board/i.test(location.href) && GM_getValue('exBoard', true)){
				...
				mainArea.innerHTML = '\
					<div id="board">\
						<form id="fboard" style="margin-bottom: 12px;" method="post" name="bpform" action='+ action +'>\
							...
							<span class="bn-flat"><input type="submit" value=" 留言 " name="bp_submit"></span><span id="bydbhp">by豆瓣精灵</span>\
			...
			for (var i=0,r=$$('.mbtrdot, .update-item .author'),l=r.length; i<l; i++){
				if(!/dbmark/.test(r[i].className)){
					r[i].innerHTML += '<span class="gact"><a title="留言到对方的留言板 by豆瓣精灵" onclick="var link=this.parentNode.parentNode.getElementsByTagName(\'a\')[0];var form=$(\'#fboard\');form.attr(\'action\',link.href+\'board\');form.find(\'input:visible\').val(\'留言到\'+link.innerHTML+\'的留言板\');form.find(\'textarea\').focus();form.find(\'a\').attr(\'style\',\'\');" href=#>回复</a></span>';
	...
	*/
				...
				$('#allPageLoder').addEventListener('click', function(){
					if(!!$$('.paginator>span.break')[0]){//超过10页
						if(!window.loadPageConfirm){//本页未提示过
							...
							confirm('检测到分页较多，可能耗时较长，是否加载？\n（当前设定为每次加载'+GM_getValue('loadPageNum', 10)+'页，可点击“豆瓣精灵”设置）') && loadPage(false, runHelper);
						}else{//本页已提示过
	...
	var Douban = /douban\.(com|fm)/i.test(location.href), doubanHelper = {name: '豆瓣精灵',id: '49911',version: '2012.11.18'};
	...
	// if(self.location == top.location && Douban){
		...
		bugFix();//修复指定页面的BUG
		FMdownLoad(); //豆瓣FM下载破解
		$('#db_BeanVine') && ($('#db_BeanVine').title = '版本：V' + doubanHelper.version + ' 运行耗时：' + (new Date() - t1)/1000 + ' 秒');//记录豆瓣精灵耗时
		// }else{
